...
% If you wish an index, you better keep these two lines
\usepackage{makeidx}
\makeindex
...
The class \ctuclsname\@ itself takes couple optional arguments, namely
\ctulst!10pt!, \ctulst!11pt! and \ctulst!12pt! for setting up the basic font size (the default is 11\,pt).

Then it takes the option \ctulst!draft! that says some draft options (the ones common in \LaTeX).
...
The parameters can be diveded into several groups.
...
\begin{itemize}
\itemM \ctulst!author! --- author's name, including titles.
\itemO \ctulst!supervisor! --- supervisor's name, including titles.
\itemO \ctulst!supervisor-address! --- supervisor's affiliation and address; use \ctulst!\\! to format it with linebreaks.
\itemO \ctulst!year!, \symbO\ctulst!month!, \ctulst!day! --- numerical values of the date of theses publishing.
\itemO \ctulst!title-<lang>! --- title of the thesis.
\item \ctulst!subtitle-<lang>! --- subtitle.
\item \ctulst!university-<lang>! --- name of the university (is preset for CTU).
\item \ctulst!university-endl-<lang>! --- name of the univesity broken in 4 lines using \ctulst!\\!.
\item \ctulst!faculty-<lang>! --- name of the faculty (set up automatically, see \ctulst!xfaculty! below).
\item \ctulst!facultynum! --- standard number of the faculty (set up automatically, see \ctulst!xfaculty! below).
\item \ctulst!doctype-<lang>! --- type of the document/thesis (set up automatically, see \ctulst!xdoctype! below).
\itemO \ctulst!department-<lang>! --- name of the department.
\itemO \ctulst!fieldofstudy-<lang>!, \ctulst!subfieldofstudy-<lang>! --- field of study.
\itemO \ctulst!keywords-<lang>! --- comma-separated list of keywords.
\end{itemize}

\paragraph{General fields}

\begin{itemize}
\itemM \ctulst!mainlanguage! --- main language of the thesis; the choices are \ctulst!czech!, \ctulst!english! and \ctulst!slovak!.
\item \ctulst!titlelanguage! --- language in which the title pages are typeset (defaults to \ctulst!mainlanguage!).
\item \ctulst!secondlanguage! --- language of the second abstract (set up automatically so that two abstracts appear: one in English and one in Czech,
 unless the thesis is in Slovakian, in which case the two abstracts are in English and in Slovakian).
\item \ctulst!otherlanguages! --- allows more languages to be loaded by \ctulst!babel! package, accepts a comma-separated list.
\itemO \ctulst!xfaculty! --- takes values \ctulst!F1!, \ctulst!F2!, \ldots, \ctulst!F8! and sets up the faculty name and number automatically.
\itemO \ctulst!xdoctype! --- takes values \ctulst!B! (bachelor), \ctulst!M! (master) or \ctulst!D! (dissertation) and sets up the document
 type automatically; for non-thesis-like documents, use \ctulst!doctype-<lang>!.
\itemO \ctulst!specification-file! --- the path to the specification file (usually something like \ctulst!zadani.pdf!,
 the current directory is searched for the file of course).
\item \ctulst!front-specification! --- if set to true, the specification will appear just after the titlepage (defaults to \ctulst!false!).
\item \ctulst!front-list-of-figures!, \ctulst!front-list-of-tables! --- says whether the list of figures/tables will be set
 in the frontmatter (default to \ctulst!true!)
\item \ctulst!monochrome! --- switches to optimized monochrome setup, in case the work will not be printed in colour.
\item \ctulst!savetoner! --- disables background and other too dark things to save toner while printing drafts.
\end{itemize}
...
\begin{itemize}
\item \ctulst!pkg-hyperref! (default: \ctulst!true!) --- sets up all the fields of the PDF document.
\item \ctulst!pkg-listings! (default: \ctulst!false!) --- sets up \ctulst!\ttfamily! font for all listings, and adds a background to all display listings.
...
\item \ctulst!pkg-amsthm! (default: \ctulst!true!) --- sets up nicely the basic styles for theorems, but does not set up the theorem environment themselves.
 More information is in Section~\ref{sect:theorems}
\item \ctulst!pkg-biblatex! (default: \ctulst!false!) --- more details in Section~\ref{sect:biblio}.
\end{itemize}
...
\begin{itemize}
\itemO \ctulst!{abstract-<lang>}! --- abstract; the language has to be always specified.
\itemO \ctulst!{thanks}! --- acknowledgements; can be used for both personal and grant ackowledgements.
\itemO \ctulst!{declaration}! --- students are oblidged to declare that the works is their works, and maybe some more things,
...
If the document is divided into parts, the appendices are introduced by a part-like title ``Appendices'', both in the document and in the table of contents.
...
\end{lstlisting}
where \ctulst!<filename>! is the root name of your main project file (without \ctulst!.tex!).
