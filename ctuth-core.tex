...
\keys_define:nn { ctuthesis } {
	...
	oneside .groups:n = { dc, nopre, nopost, nop },
	oneside .initial:n = { false },
	twoside .meta:n = { oneside = false },
	twoside .groups:n = { dc, nopre, nopost, nop },
	
	% Draft option
	draft .bool_gset:c = { g_ctuthesis_switch_draft_bool },
	draft .groups:n = { dc, nopre, nopost, nop },
	...
	% the value of preprint will appear in the foot of every page
	preprint .tl_gset:c = g_ctuthesis_field_preprint_tl,
	preprint .groups:n = { dc, pre, post },
%	preprint .initial:n = { bla },

}

% Switches defaulting to false
\tl_map_inline:Nn { {front-specification}{monochrome} } {
	\keys_define:nn { ctuthesis } {
...
\tl_map_inline:Nn { {front-list-of-tables}{front-list-of-figures} } {
	\keys_define:nn { ctuthesis } {
		#1 .bool_gset:c = { g_ctuthesis_switch_#1_bool },
		#1 .groups:n = { dc, pre, post },
		#1 .initial:n = { true },
...
\tl_map_inline:Nn { {10pt}{11pt}{12pt} } {
	\keys_define:nn { ctuthesis } {
		#1 .meta:n = { fontsize = #1 },
		#1 .groups:n = { dc, nopre, nopost, nop },
	}
}
...
\NewDocumentCommand \ctusetup { m } {
	% 'nop' group keys are forbidden. We collect them and if there are any, we invoke the error 'keys-nopre'
...
\NewDocumentCommand \ctusetup@postprocess { m } {
	% 'nopost' group keys are forbidden. We collect them and if there are any, we invoke the error 'keys-nopre'
	\keys_set_filter:nnnN { ctuthesis } { nopost } { #1 } \l_temp_ctuthesis_a_tl
  	\group_begin:
  		% The calls of \keys_set_filter here should actually not set anything, but better behave like
  		% the non-existant \keys_filter. This is done by disabling any action of \@@_set_elt_aux:,
  		% which is responsible for the key setting. This works certainly with expl3.sty 2014/11/25 v5471.
  		% A feature request to the LaTeX3 team will be sent.
	  	\cs_set:Nn \@@_set_elt_aux: {}
	  	% Store 'nop' keys in one variable and 'pre' keys in another one.
		\keys_set_filter:nnVN { ctuthesis } { nop } \l_temp_ctuthesis_a_tl \l_temp_ctuthesis_b_tl
		\tl_gset_eq:NN \g_temp_ctuthesis_b_tl \l_temp_ctuthesis_b_tl
		\keys_set_filter:nnVN { ctuthesis } { pre } \l_temp_ctuthesis_a_tl \l_temp_ctuthesis_c_tl
		\tl_gset_eq:NN \g_temp_ctuthesis_c_tl \l_temp_ctuthesis_c_tl
	\group_end:
	% Use \keys_set to the special key modules to call the error functions
	\keys_set:nV { ctuthesis-nop } \g_temp_ctuthesis_b_tl
	\keys_set:nV { ctuthesis-nopost } \g_temp_ctuthesis_c_tl
	% Trigger called after any \keys_set
	\ctuthesis_ctusetup_trigger:
}
...
\IfFileExists{.git}{\typeout{GIT TRUE}}{\typeout{GIT FALSE}}
